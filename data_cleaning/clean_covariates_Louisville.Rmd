---
title: "clean_covariates_Louisville"
...
```{r}
# set data path
DATA_PATH <- "C:/Users/rajak/OneDrive - UCLA IT Services/UCLA/S25/POL SCI 200C/Project"
```
...
```{r}
AGE_PATH <- file.path(DATA_PATH, "Louisville_age")
...
```{r}
EMPLOYMENT_PATH <- file.path(DATA_PATH, "Louisville_employment")
...
```{r}
SEX_PATH <- file.path(DATA_PATH, "Louisville_sex")
...
```{r}
louisville_covariates <- inner_join(age_df, emp_df, by = "year")
louisville_covariates <- inner_join(louisville_covariates, sex_df, by = "year")
louisville_covariates
```
```{r}
write_csv(louisville_covariates, "louisville_covariates.csv")
```